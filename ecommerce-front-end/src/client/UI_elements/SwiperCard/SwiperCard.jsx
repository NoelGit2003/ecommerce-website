        ...
        <SwiperSlide key={card._id}>
          <div className="card">
            <div className="card-details">
              <NavLink to={`/productDetails/${card._id}`} style={{textDecoration: 'none'}}>
                <img src={`http://localhost:3000/uploads/${card.ProductImage}`} alt="collection_image" className="card-image" />
                <div className="card-name" >{card.ProductName}</div>
                <div className="card-price">
                  <span>{currency}</span>
                  <span>{card.ProductPrice}</span>
                </div>
                <div className="card-rating">
                  <span className='star-rating'>
                    {renderStars(card.ProductRating)}
                  </span>
                  <span className="card-rating-number">{card.ProductRating}</span>
                </div>
              </NavLink>
            </div>
          </div>
